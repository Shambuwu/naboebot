...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const battlecatsList = await db.getAllBattlecatsByUser(message.author.id, message.guild.id)
        const battlecats = battlecatsList.sort((a, b) => JSON.parse(a.stats).lvl > JSON.parse(b.stats).lvl ? -1 : 1).map(row => ({name: row.name, value: `Level: ${JSON.parse(row.stats).lvl}\n${JSON.parse(row.stats).rarity}`, inline: true}))
